...
for(var i = 0; i < min(ds_list_size(grabbed_ids),15); i++) {
	with(ds_list_find_value(grabbed_ids,i)) {
		if(!defeated && !pulled) {
				...
				var boost = 1;
				if(lifetime_modif) {
					boost = 1+5*(-other.lifetime-20)/60;
				}
				t_axis_x *= 0.2;
				t_axis_y *= 0.2;
				movement_vector_add(other.pull_power*distance_to_blackhole*0.5*boost+0.4,point_direction(x,y,other.x,other.y)-(dsin(other.rot_angle*3)*0.25+1)*(30+120*(0.6-distance_to_blackhole)));
				pulled = true;
			}
		}
	}
